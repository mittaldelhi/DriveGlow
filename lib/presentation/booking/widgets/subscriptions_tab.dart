...
class SubscriptionsTab extends ConsumerWidget {
  ...
  List<BookingModel> _getBookedServices(String planId, String? vehicleNumber) {
    ...
    return bookings.where((b) {
      if (b.vehicleNumber?.toLowerCase() != vehicleNumber.toLowerCase())
        return false;
      ...
      // Check if it's today
      if (b.appointmentDate.isBefore(today) ||
          b.appointmentDate.isAfter(tomorrow))
        return false;
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) =>
            _buildSubscriptionCard(context, ref, summaries[index]),
      ),
    );
  }
  ...
  List<SubscriptionSummary> _buildSubscriptionSummaries() {
    ...
    final subscriptions = bookings.where((b) {
      final refData = parseBookingRef(b.serviceId);
      // Check: is subscription booking, not cancelled, and not expired
      final isSubscription = refData.type == 'subscription';
      final isNotCancelled = b.status != BookingStatus.cancelled;
      final isNotExpired =
          b.subscriptionPeriodEnd == null ||
          b.subscriptionPeriodEnd!.isAfter(now);
      return isSubscription && isNotCancelled && isNotExpired;
    ...
    return latestByKey.values.map((subscription) {
      ...
      final planName = refData.name.isNotEmpty
          ? refData.name
          : (plan?['name'] as String? ?? 'Subscription Plan');

      int? maxAllowed;
      final limitsRaw = plan?['service_usage_limits'] as Map<String, dynamic>?;
      if (limitsRaw != null && limitsRaw.isNotEmpty) {
        maxAllowed = limitsRaw.values.fold<int>(
          0,
      ...
      final included =
          (plan?['included_service_ids'] as List?)
              ?.map((e) => e.toString())
              .toList() ??
          <String>[];
      ...
      if (limitsMap != null && limitsMap is Map) {
        serviceLimits = Map<String, int>.from(
          limitsMap.map(
  ...
  Widget _buildSubscriptionCard(
    ...
    final validityText =
        '${validityDate.day} ${_getMonthName(validityDate.month)} ${validityDate.year}';
    ...
    return Column(
      ...
      children: [
        ...
        if (bookedServices.isEmpty)
          ...
          ...bookedServices.map(
            (booking) => _buildBookedServiceCard(context, ref, booking),
          ),
      ],
    );
  }

  Widget _buildBookedServiceCard(
    BuildContext context,
    WidgetRef ref,
    BookingModel booking,
  ) {
    final refData = parseBookingRef(booking.serviceId);
    final serviceName = refData.name.isNotEmpty
        ? refData.name
          ...
          Row(
            ...
            children: [
              Text(
                DateFormat(
                  'dd MMM yyyy, hh:mm a',
                ).format(booking.appointmentDate.toLocal()),
  ...
  Future<void> _rescheduleBooking(
    ...
    final pickedDate = await showDatePicker(
      context: context,
      initialDate: booking.appointmentDate.isAfter(DateTime.now())
    ...
    try {
      await ref
          .read(bookingRepositoryProvider)
          .rescheduleBooking(bookingId: booking.id, appointmentDate: newDate);
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
